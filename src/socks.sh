...
socks_user_config() {
	echo
	while :; do
		read -p "$(echo -e "请输入$yellow用户名$none...(默认用户名: ${cyan}goxianguo$none)"): " new_socks_username
		[ -z "$new_socks_username" ] && new_socks_username="goxianguo"
...
socks_pass_config() {
	echo
	while :; do
		read -p "$(echo -e "请输入$yellow密码$none...(默认密码: ${cyan}goxianguo.com$none)"): " new_socks_userpass
		[ -z "$new_socks_userpass" ] && new_socks_userpass="goxianguo.com"
